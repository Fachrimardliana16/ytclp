...
const { scoreHighlights, generateTitle, suggestCaptionStyle, generateBatchTitles, getAIStatus } = require('./openrouter');

const router = express.Router();

// ===== AI-powered highlight scoring (auto-fallback ke local) =====
router.post('/analyze', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ highlights, mode: status.activeMode, model: status.model });
  } catch (err) {
...
// ===== AI title generation (auto-fallback) =====
router.post('/title', requireAuth, async (req, res) => {
  try {
    ...
    const status = await getAIStatus();
    res.json({ title, mode: status.activeMode });
  } catch (err) {
...
// ===== AI caption style suggestion (auto-fallback) =====
router.post('/caption-style', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ style, mode: (await getAIStatus()).activeMode });
  } catch (err) {
...
// ===== Batch title generation (auto-fallback) =====
router.post('/titles', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ titles, mode: (await getAIStatus()).activeMode });
  } catch (err) {
...
// ===== Smart crop analysis (unchanged — ffmpeg-based) =====
